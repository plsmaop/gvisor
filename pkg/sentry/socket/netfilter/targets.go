...
func init() {
	...
	// Error targets include user chains in addition to actual errors.
...
func (redirectTargetMaker) marshal(target stack.Target) []byte {
	...
	xt.NfRange.RangeIPV4.MinPort = binary.BigEndian.Uint16(port)
	binary.LittleEndian.PutUint16(port, rt.MaxPort)
	xt.NfRange.RangeIPV4.MaxPort = binary.BigEndian.Uint16(port)
	return binary.Marshal(ret, usermem.ByteOrder, xt)
}
