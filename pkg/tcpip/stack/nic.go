...
type NIC struct {
	...
	atomics struct {
		...
		// the enabled status without worrying about currently held locks and
		// synchronize writes when enabling/disabling the NIC.
		enabled uint32
	}
...
func (n *NIC) Enabled() bool {
	return atomic.LoadUint32(&n.atomics.enabled) == 1
}
...
func (n *NIC) setEnabled(v bool) bool {
	if v {
		return atomic.SwapUint32(&n.atomics.enabled, 1) == 0
	}
	return atomic.SwapUint32(&n.atomics.enabled, 0) == 1
}
