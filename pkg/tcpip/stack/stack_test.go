...
func TestGetNetworkEndpoint(t *testing.T) {
	...
	protocols := make([]stack.NetworkProtocol, 0, len(tests))
	for _, test := range tests {
		protocols = append(protocols, test.proto)
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			protoNum := test.proto.Number()

			ep, err := s.GetNetworkEndpoint(nicID, protoNum)
			if err != nil {
				t.Fatalf("s.GetNetworkEndpoint(%d, %d): %s", nicID, protoNum, err)
			}

			if got := ep.NetworkProtocolNumber(); got != protoNum {
				t.Fatalf("got ep.NetworkProtocolNumber() = %d, want = %d", got, protoNum)
			}
		})
	}
}
