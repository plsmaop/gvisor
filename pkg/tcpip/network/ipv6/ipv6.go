...
func (e *endpoint) setEnabled(v bool) bool {
	if v {
		return atomic.SwapUint32(&e.atomics.enabled, 1) == 0
	}
	return atomic.SwapUint32(&e.atomics.enabled, 0) == 1
}
...
func (e *endpoint) stopDADForPermanentAddressesLocked() {
	...
	e.mu.addressableEndpointState.ReadOnly().ForEach(func(addressEndpoint stack.AddressEndpoint) bool {
		if addr := addressEndpoint.AddressWithPrefix().Address; addressEndpoint.GetKind() == stack.PermanentTentative && header.IsV6UnicastAddress(addr) {
...
//
// addAndAcquirePermanentAddressLocked will also join the passed address's
...
func (e *endpoint) hasPermanentAddressRLocked(addr tcpip.Address) bool {
	ep := e.getAddressRLocked(addr)
	if ep == nil {
		return false
	}
	return ep.GetKind().IsPermanent()
}
...
func (p *protocol) setForwarding(v bool) bool {
	if v {
		return atomic.SwapUint32(&p.atomics.forwarding, 1) == 0
	}
	return atomic.SwapUint32(&p.atomics.forwarding, 0) == 1
}
