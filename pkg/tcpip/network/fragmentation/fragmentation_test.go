...
func TestFragmentationProcess(t *testing.T) {
	for _, c := range processTestCases {
		t.Run(c.comment, func(t *testing.T) {
			f := NewFragmentation(minBlockSize, 1024, 512, DefaultReassembleTimeout, faketime.NewNullClock())
...
func TestReassemblingTimeout(t *testing.T) {
	...
	// event specifies both a fragment to process and a duration to advance the
	// clock before the fragment is processed (if the fragment is nil, only the
	// clock advances). sizeAfterEvent is the expected size of the fragmentation
	...
	tests := []struct {
		...
		},
		{
			...
			events: []event{
				...
				{
					name:           "reassemblyTImeout (1)",
					clockAdvance:   reassemblyTimeout,
					sizeAfterEvent: 0,
				},
				{
					name:           "half2",
...
func TestMemoryLimits(t *testing.T) {
	f := NewFragmentation(minBlockSize, 3, 1, DefaultReassembleTimeout, faketime.NewNullClock())
...
func TestMemoryLimitsIgnoresDuplicates(t *testing.T) {
	f := NewFragmentation(minBlockSize, 1, 0, DefaultReassembleTimeout, faketime.NewNullClock())
	// Send first fragment with id = 0.
...
func TestErrors(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			f := NewFragmentation(test.blockSize, HighFragThreshold, LowFragThreshold, DefaultReassembleTimeout, faketime.NewNullClock())
